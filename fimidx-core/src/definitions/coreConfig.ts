...
export const coreConfigSchema = z.object({
  postgres: z.object({
    url: z.string({ message: `${envVars.FIMIDX_POSTGRES_URL} is not set` }),
  }),
  turso: z.object({
    url: z
  ...
  resend: z.object({
    fromEmail: z.string({ message: `${envVars.RESEND_FROM_EMAIL} is not set` }),
  ...
  fimidara: z.object({
    ...
    rootname: z.string({ message: `${envVars.FIMIDARA_ROOTNAME} is not set` }),
    /**
  ...
  unzipSourceMaps: z.object({
    url: z
      ...
      .url({ message: `${envVars.UNZIP_SOURCE_MAPS_URL} is not a valid URL` }),
    intervalMs: z.coerce
      .number()
      .optional()
      .default(1000 * 60 * 10), // 10 minutes
